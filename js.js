...
function App() {
  ...
  this.showMessage = function(message, description, alert){
    ...
    if(description)
      $('#modal-message-description').text(description);
    else
...
function BookingManager(settings){
    ...
    var ViewModel = function(){
      ...
      this.searchResources = function() {
        var start = moment.utc(vm.startDate()).format('YYYY-MM-DDT') + moment(vm.startTime()).format('HH:mm') + 'Z';
        var end = moment.utc(vm.endDate()).format('YYYY-MM-DDT') + moment(vm.endTime()).format('HH:mm')  + 'Z';
...
ko.bindingHandlers.pickadate = {
    ...
    update: function(element, valueAccessor, allBindings, viewModel, bindingContext) {
        ...
        $input.pickadate('set', { select: date });
...
ko.bindingHandlers.pickatime = {
    init: function(element, valueAccessor, allBindings, viewModel, bindingContext) {
        ...
        //init time picker
        $input.pickatime(valueUnwrapped.options);
        ...
        $input.pickatime('on', {
          ...
          set: function(thingSet) {
            if(!isNaN(thingSet.select) && thingSet.select >= 0){
              ...
              var currentDate = moment(valueAccessor().value()).startOf('day');
              currentDate = currentDate.add(thingSet.select,'minutes').toDate();

              //If the date has changed, update the observable
              if(valueAccessor().value().getTime() != currentDate.getTime())
